...
def main(config):
    ...
    tokenizer = BertTokenizer.from_pretrained('./bert-base-chinese/vocab.txt', do_lower_case=True)
    train_examples = processor.get_train_examples(args.data_dir)
    eval_examples = processor.get_test_examples(args.data_dir)
    num_train_optimization_steps = int(
        len(train_examples) / args.train_batch_size / args.gradient_accumulation_steps) * args.num_train_epochs
    bert_base_model = BertModel.from_pretrained('./bert-base-chinese')
    bert_base_model.config.num_labels = num_labels

    if args.dataset in {'camera', 'car', 'phone', 'notebook', 'all'}:
...
def parse_experiments(path):
    ...
    for id, config in json_config.items():
        ...
        parser.add_argument("--eval_steps", default=100, help="evaluate per steps")
